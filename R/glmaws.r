...
#
#    this version tries to be as close to the local polynomial code as possible
#    for the Poisson and Bernoulli model regularization is done by modifying y
#    Poisson: add  1/(2*#U(i))
#    Bernoulli:  replace  1  by  1-1/(2*#U(i))  replace  0 by    1/(2*#U(i))
#
...
#
if(degree==0) return("use aws for local constant models")
if(degree>3)  return("no defaults for parameters available")
...
n2 <- dy[2]
n <- n1*n2
if(degree>2) return("bivariate aws on a grid is not implemented for degree>2")
...
lines(switch(mfamily,Gaussian=tobj$theta[1,],Poisson=exp(tobj$theta[1,]),
                       Bernoulli=exp(tobj$theta[1,])/(1+exp(tobj$theta[1,])),
                       Exponential=1/tobj$theta[1,]),lwd=2)
if(conf) {
    lines(switch(mfamily,Gaussian=confint[1,],Poisson=exp(confint[1,]),
                       Bernoulli=exp(confint[1,])/(1+exp(confint[1,])),
                       Exponential=1/confint[1,]),lwd=1,col=4)
    lines(switch(mfamily,Gaussian=confint[2,],Poisson=exp(confint[2,]),
