...
#             Volatility:    qlambda=.98       
#
caws0 <- function(y,qlambda=NULL,lkern="Triangle",aggkern="Uniform",
                 family="Gaussian",sigma2=NULL,shape=NULL,hinit=NULL,hincr=NULL,hmax=NULL,
		 heta=NULL,qtau=NULL,eta0=NULL,NN=FALSE,u=NULL,
                 graph=FALSE,demo=FALSE,wghts=NULL)
{
IQRdiff <- function(y) IQR(diff(y))/1.908
KLdist <- function(mcode,th1,th2,bi0){
...
gc()
dim(zobj$ai)<-dy
if(hakt>n1/2) zobj$bi0 <- hincr*hincr*hincr*biold
biold <- zobj$bi0
...
###         
z<-list(theta=tobj$theta,ni=tobj$bi,var=tobj$bi2/tobj$bi^2*sigma2,y=y,x=x,call=args)
class(z)<-switch(family,Gaussian="laws.gaussian",Bernoulli="laws.bernoulli",Exponential="laws.exponential",
                 Poisson="laws.poisson",Weibull="laws.weibull",Volatility="laws.vola")
z
}
