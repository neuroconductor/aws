...
#    Volatility models            
#    with modification of location weights depending on previous results
#
...
#       
awsw <- function(y,hmax=NULL,aws=TRUE,memory=FALSE,family="Gaussian",
                ...
                testprop=FALSE,maxni=FALSE,wadj=1.25,wexp=0)
{
  #
  ...
  wghts <- array(1,c(6,n1,n2,n3))
  wdir <- c(1,1,1)
  wdir[d:2] <- 0
  while (k<=kstar) {
    hakt0 <- gethani(1,1.25*hmax,lkern,1.25^(k-1),wdir,1e-4)
    hakt <- gethani(1,1.25*hmax,lkern,1.25^k,wdir,1e-4)
    ...
    # Correction for spatial correlation depends on h^{(k)} 
      # all other cases
        zobj <- .Fortran("cawsw2io",as.double(y),
                         as.logical(tobj$fix),
                         as.integer(n1),
                         as.integer(n2),
                         as.integer(n3),
                         ...
                         double(prod(dlw)),
                         as.double(wghts),
                         outwghts = double(6*n1*n2*n3),
                         PACKAGE="aws")[c("bi","bi0","bi2","ai","hakt","hhom","outwghts")]
    wghts <- array(zobj$outwghts,c(6,n1,n2,n3))
    for(ii in 1:6) wghts[ii,,,] <- wghts[ii,,,]*(1+wadj/zobj$bi^wexp)
