vpaws <- function(y,kstar=16,
   ...
   while (k<=kstar) {
      ...
      zobj <- .Fortran("vpaws",as.double(y),
                       as.logical(mask),
                       as.integer(nvec),
                       as.integer(n1),
                       as.integer(n2),
...
vpawsfull <- function(y,kstar=16,
                 invcov=NULL,mask=NULL,scorr=0,spmin=0.25,
                 ladjust=1,wghts=NULL,u=NULL,
                 maxni=FALSE,patchsize=1){
##
...
##
   ...
   d <- length(dy)
   if(length(dy)>3) stop("Vector AWS for more than 3 dimensional grids is not implemented")
   lambda <- 2*ladjust*switch(d,qchisq(pchisq(14.6,1),nvec),## 1D
                      qchisq(pchisq(9.72,1),nvec),## 2D
                      qchisq(pchisq(8.82,1),nvec))## 3D
   if(is.null(wghts)) wghts <- c(1,1,1)
   wghts <- switch(length(dy),c(0,0),c(wghts[1]/wghts[2],0),wghts[1]/wghts[2:3])
   ...
   while (k<=kstar) {
      ...
      dlw<-(2*trunc(hakt/c(1,wghts))+1)[1:d]
      if(scorr[1]>=0.1) lambda0<-lambda0*Spatialvar.gauss(hakt0/0.42445/4,h0,d)/Spatialvar.gauss(hakt0/0.42445/4,1e-5,d)
      zobj <- .Fortran("pvaws2",as.double(y),
                       as.logical(mask),
                       as.integer(nvec),
                       ...
                       as.integer(np3),
                       double(nvec*np1*np2*np3*mc.cores),
                       double(nvec*(nvec+1)/2*np1*np2*np3*mc.cores),
                       double(np1*np2*np3*mc.cores),
                       PACKAGE="aws")[c("bi","theta","hakt")]
      dim(zobj$theta)<-c(nvec,dy)
      if(maxni) bi <- zobj$bi <- pmax(bi,zobj$bi)
