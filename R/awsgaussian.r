...
#
    if (cluster==TRUE) {
      if (is.null(levels)) {
        tobj <- awssegment2(tobj,nlevels,swghts,rhoseq[k])
      } else {
        tobj <- awssegment2(tobj,levels,swghts,rhoseq[k])
      }
    } else {
...
awssegment2 <- function(tobj,levels,swghts,rho) {
  theta <- tobj$theta
  dy <- dim(theta)
  dim(theta) <- NULL
  cl <- kmeans(theta,levels)
  levels <- cl$centers
  ind <- cl$cluster
  theta <- rho * levels[ind] + (1-rho) * theta
  dim(theta) <- dim(ind) <- dy
