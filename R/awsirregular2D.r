aws.irreg2D <- function(y,x,hmax=NULL,hpre=NULL,qlambda=NULL,qtau=NULL,varmodel="Constant",
                sigma2=NULL,varprop=.1,graph=FALSE,
		lkern="Triangle",aggkern="Uniform",
		spmin=0,lseq=NULL,nbins=100,binsize=NULL,border=0,henv=NULL)
{
#
...
ry <- range(x[,2])
if(is.null(binsize)) {
binsize <- min(diff(rx)/nbins,diff(ry)/nbins)
} 
border <- max(0,border)
breaks.x <- seq(rx[1]-border*binsize,rx[2]+border*binsize,binsize)
breaks.y <- seq(ry[1]-border*binsize,ry[2]+border*binsize,binsize)
zbins<-binning.2d(x[,1],x[,2],y,breaks.x,breaks.y)
ni <- zbins$table.freq
nbins <- dim(ni)
mask <- ni>0 
...
#
#
#   2D binning (adapted from sm-library)
#
#
binning.2d <- function (x, y, z, breaks.x, breaks.y){
        f1 <- cut(x, breaks = breaks.x)
        f2 <- cut(y, breaks = breaks.y)
        freq <- table(f1, f2)
        dimnames(freq) <- NULL
        nbins <- dim(freq)
        midpoints.x <- (breaks.x[-1] + breaks.x[-(nbins[1] + 1)])/2
        midpoints.y <- (breaks.y[-1] + breaks.y[-(nbins[2] + 1)])/2
        z1 <- midpoints.x
        z2 <- midpoints.y
        ...
        X.f <- X.f[id]
        result <- list(x = X, x.freq = X.f, midpoints.x = midpoints.x, midpoints.y = midpoints.y,
            breaks.x = breaks.x, breaks.y=breaks.y, table.freq = freq)
        if (!all(is.na(z))) {
            result$means <- as.numeric(tapply(z, list(f1, f2),
                mean))[id]
            result$devs <- as.numeric(tapply(z, list(f1, f2),
