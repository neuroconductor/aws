...
ckernsm <- function (y, h = 1, lkern = "Triangle")
{
  ...
  if (length(h) == 1)
    h <- rep(h, d)
  if (length(h) != d)
    stop("Incompatible length of bandwidth vector h")
  if (d == 1) {
    z <- extend.y(y, h[1], 1)
    yy <- z$yy
    dyy <- length(yy)
    kern <-
      switch(
        lkern,
        Uniform = as.numeric(abs(grid(dyy, h[1])) < 1),
        Triangle = pmax(1 - grid(dyy, h[1]) ^ 2, 0),
        Quadratic = pmax(1 - grid(dyy, h[1]) ^ 2, 0) ^
          2,
        Cubic = pmax(1 - grid(dyy, h[1]) ^ 2, 0) ^ 3
      ) /
      switch(lkern, h[1] * beta(.5, switch(
        lkern,
        Uniform = 1,
        Triangle = 2,
        Quadratic = 3,
    ...
    bi <- sum(kern)
    yhat <- Re(fft(fft(yy) * fft(kern), inverse = TRUE))[z$ind] / dyy /
      bi
    bi <- bi * switch(lkern, h[1] * beta(.5, switch(
      lkern,
      Uniform = 1,
      Triangle = 2,
      Quadratic = 3,
  ...
  if (d == 2) {
    ...
    kern1 <-
      switch(
        lkern,
        Uniform = as.numeric(abs(grid(dyy1, h[1])) < 1),
        Triangle = pmax(1 - grid(dyy1, h[1]) ^ 2, 0),
        Quadratic = pmax(1 - grid(dyy1, h[1]) ^ 2, 0) ^
          2,
        Cubic = pmax(1 - grid(dyy1, h[1]) ^ 2, 0) ^
          3
      ) /
      switch(lkern, h[1] * beta(.5, switch(
        lkern,
    ...
    yhat <-
      t(Re(mvfft(mvfft(yy) * fft(kern1), inverse = TRUE))[z$ind, ] / dyy1 / sum(kern1))
    ...
    kern2 <-
      switch(
        lkern,
        Uniform = as.numeric(abs(grid(dyy2, h[2])) < 1),
        Triangle = pmax(1 - grid(dyy2, h[2]) ^ 2, 0),
        Quadratic = pmax(1 - grid(dyy2, h[2]) ^ 2, 0) ^
          2,
        Cubic = pmax(1 - grid(dyy2, h[2]) ^ 2, 0) ^
          3
      ) /
      switch(lkern, h[2] * beta(.5, switch(
        lkern,
        Uniform = 1,
        Triangle = 2,
        Quadratic = 3,
    ...
    yhat <-
      t(Re(mvfft(mvfft(yy) * fft(kern2), inverse = TRUE))[z$ind, ] / dyy2 / sum(kern2))
    bi <-
      sum(outer(kern1, kern2)) * switch(lkern, h[1] * h[2] * beta(.5, switch(
        lkern,
        Uniform = 1,
        Triangle = 2,
        Quadratic = 3,
  ...
  if (d == 3) {
    ...
    yhat <-
      Re(mvfft(mvfft(yy) * fft(kern1), inverse = TRUE))[z$ind, ] / dyy1 / sum(kern1)
    dim(yhat) <- dy
    ...
    yhat <-
      Re(mvfft(mvfft(yy) * fft(kern2), inverse = TRUE))[z$ind, ] / dyy2 / sum(kern2)
    dim(yhat) <- c(dy[2], dy[1], dy[3])
    ...
    kern3 <-
      switch(
        lkern,
        Uniform = as.numeric(abs(grid(dyy3, h[3])) < 1),
        Triangle = pmax(1 - grid(dyy3, h[3]) ^ 2, 0),
        Quadratic = pmax(1 - grid(dyy3, h[3]) ^ 2, 0) ^
          2,
        Cubic = pmax(1 - grid(dyy3, h[3]) ^ 2, 0) ^
          3
      ) /
      switch(lkern, h[2] * beta(.5, switch(
        lkern,
    ...
    yhat <-
      Re(mvfft(mvfft(yy) * fft(kern3), inverse = TRUE))[z$ind, ] / dyy3 / sum(kern3)
    dim(yhat) <- c(dy[3], dy[1], dy[2])
    yhat <- aperm(yhat, c(2, 3, 1))
    bi <-
      sum(outer(outer(kern1, kern2), kern3)) * switch(lkern, h[1] * h[2] * h[3] *
                                                        beta(.5, switch(
                                                          lkern,
                                                          Uniform = 1,
                                                          Triangle = 2,
...
gkernsm <- function (y, h = 1)
{
  ...
  if (d == 2) {
    ...
    yhat <-
      t(Re(mvfft(mvfft(yy) * fft(kern2), inverse = TRUE))[z$ind, ] / dyy2 / sum(kern2))
    bi <-
      sum(outer(kern1, kern2)) / dnorm(0, 0, 2 * h[1] / dyy1) / dnorm(0, 0, 2 * h[2] /
                                                                        dyy2)
  }
  if (d == 3) {
    dim(y) <- c(dy[1], dy[2] * dy[3])
    ...
    yhat <- aperm(yhat, c(2, 3, 1))
    bi <-
      sum(outer(outer(kern1, kern2), kern3)) / dnorm(0, 0, 2 * h[1] / dyy1) /
      dnorm(0, 0, 2 * h[2] / dyy2) / dnorm(0, 0, 2 * h[3] / dyy3)
  }
  bi <- array(bi, dim(y))
  list(theta = yhat, bi = bi)
}
Varcor <- function(lkern, h, d = 1) {
  ...
  x <- ((-ih):ih) / h
  if (d == 2)
    x <- sqrt(outer(x ^ 2, x ^ 2, "+"))
  if (d == 3)
    x <- sqrt(outer(x ^ 2, outer(x ^ 2, x ^ 2, "+"), "+"))
  penl <- switch(
    lkern,
    Triangle = pmax(0, 1 - x ^ 2),
...
SpatialCorr <- function(lkern, h, d = 1) {
  ...
  dim(penl) <- rep(dx, d)
  if (d == 1)
    z <- sum(penl[-1] * penl[-dx]) / sum(penl ^ 2)
  if (d == 2)
    z <- sum(penl[-1, ] * penl[-dx, ]) / sum(penl ^ 2)
  if (d == 3)
    z <- sum(penl[-1, , ] * penl[-dx, , ]) / sum(penl ^ 2)
...
gfft <- function (y, h = 1)
{
  ...
  if (length(dy) == 1) {
    ...
    yhat <-
      Re(fft(fft(yy) * fft(kern), inverse = TRUE))[z$ind] / dy / sum(kern)
  }
  if (length(dy) == 2) {
    z <- extend.y(y, h, 2)
    yy <- z$yy
    ...
    yhat <-
      t(Re(mvfft(mvfft(yy) * fft(kern), inverse = TRUE))[z$ind, ] / dy[1] / sum(kern))
    z <- extend.y(yhat, h, 2)
    yy <- z$yy
    dy <- dim(yy)
    kern <- dnorm(grid(dy[1]), 0, 2 * h / dy[1])
...
gethseq <- function(awsobj) {
  ...
  h <- numeric(kstar)
  for (i in k:kstar)
    h[i] <- gethani(1, 1.25 * hmax, lkern, 1.25 ^ i, wghts, 1e-4)
  h
}
